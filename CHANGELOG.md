...
### Compatibility
- CI covers **every Ruby × Rails combination the gemspec permits** - 28 cells. `railties` declares `>= 2.5` (6.1), `>= 2.7` (7.0, 7.1), `>= 3.1` (7.2) and `>= 3.2` (8.0, 8.1), none of them declare an upper bound, and this gem requires Ruby `>= 3.1`.
  ...
  \* Permitted by the declared constraints but beyond what Rails itself documents as supported. These cells run and report, but do not gate the build.
- **Rails 8.1 and Ruby 4.0 are covered.** Rails 8.1 is what the default lockfile resolves to, so the suite had been running against it without saying so. Ruby 4.0 has been the current stable release since July 2026 and is permitted by Rails 8.1's own `>= 3.2.0`.
